...
/* =========================================================
 * DIAGNÓSTICO TEMPORÁRIO — traduções
 *
 * Mostra, apenas a administradores com sessão iniciada, o que o
 * WordPress está realmente a ver ao carregar as traduções do tema.
 * REMOVER assim que o problema estiver resolvido.
 * ========================================================= */
add_action( 'wp_footer', function () {
    ...
    $mo     = $dir . 'scanpro-child-' . $locale . '.mo';
    $php    = $dir . 'scanpro-child-' . $locale . '.l10n.php';

    $found = glob( $dir . 'scanpro-child-*' );
    $found = $found ? array_map( 'basename', $found ) : [];
    ...
    $rows = [
        ...
        'textdomain carregado' => is_textdomain_loaded( 'scanpro-child' ) ? 'SIM' : 'NAO',
        'WP_LANG_DIR'          => WP_LANG_DIR,
        '.mo esperado'         => basename( $mo ) . ( file_exists( $mo ) ? ' (existe, ' . filesize( $mo ) . ' B)' : ' — NAO EXISTE' ),
        '.l10n.php esperado'   => basename( $php ) . ( file_exists( $php ) ? ' (existe, ' . filesize( $php ) . ' B)' : ' — NAO EXISTE' ),
        'ficheiros na pasta'   => $found ? implode( ', ', $found ) : '(nenhum)',
        'TRADUCAO APLICADA'    => ( $result !== $probe ) ? 'SIM' : 'NAO',
        'texto devolvido'      => mb_substr( $result, -40 ),
    ];

    echo '<div style="position:fixed;bottom:0;left:0;right:0;z-index:99999;background:#111;color:#0f0;'
       . 'font:12px/1.5 monospace;padding:12px 16px;max-height:45vh;overflow:auto;">';
    echo '<strong style="color:#fff">DIAGNOSTICO DE TRADUCOES — visivel so para administradores</strong><br>';
    foreach ( $rows as $k => $v ) {
        echo esc_html( $k ) . ': <span style="color:#fff">' . esc_html( (string) $v ) . '</span><br>';
    }
    echo '</div>';
} );
